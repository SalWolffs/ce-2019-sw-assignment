...
static const fe25519 ge25519_ecd = {{0x5978A3, 0x4DCA13, 0xAB75EB, 0x4141D8,
                                     0x700A4D, 0xE89800, 0x797779, 0x8CC740,
                                     0x6FFE73, 0x6CEE2B, 0x5203}};
/* 2*d */
static const fe25519 ge25519_ec2d = {{0xB2F159, 0x9B9426, 0x56EBD6, 0x8283B1,
                                      0xE0149A, 0xD13000, 0xF2EEF3, 0x198E80,
                                      0xDFFCE7, 0xD9DC56, 0x2406}};

static const fe25519 ge25519_magic = {{0xA2BF03, 0x02557F, 0x416637, 0xA5BE8D,
                                       0xD0E9E8, 0x27BF62, 0x6E01FE, 0xE93D84,
                                       0x50035D, 0x76FA30, 0x0793}};

const group_ge group_ge_neutral = {{{0}}, {{1}}, {{1}}, {{0}}};
...
const group_ge group_ge_base = {
    {{0x25D51A, 0x2D608F, 0xB2C956, 0x9525A7, 0x2CC760, 0xDC5C69, 0x31FDD6,
      0xC0A4E2, 0x6E53FE, 0x36D3CD, 0x2169}},
    {{0x666658, 0x666666, 0x666666, 0x666666, 0x666666, 0x666666, 0x666666,
      0x666666, 0x666666, 0x666666, 0x6666}},
    {{0x000001, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000,
      0x000000, 0x000000, 0x000000, 0x0000}},
    {{0xB7DDA3, 0x8AB3A5, 0xF56DDE, 0x775152, 0xF09F80, 0xE37D20, 0x8E64AB,
      0x66EA4E, 0x8B7665, 0x5F0FD7, 0x6787}}};
...
void group_ge_add_index(group_ge *gout, const group_ge *gin, int index) {
    ...
    uint32_t result[sizeof(group_ge) >> 2] = {0};
    uint32_t *in = (uint32_t*)gin;

    for (i = 0; i < WINDOWMASK + 1; i++) {
        mask = -(((i ^ index) - 1) >> 31);

        for (j = 0; j < sizeof(group_ge) >> 2; j++) {
